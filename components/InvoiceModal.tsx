  ...
  return (
    <div className="fixed inset-0 bg-black/85 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
      <div className="bg-gray-800 rounded-[2.5rem] w-full max-w-lg shadow-2xl border border-gray-700 animate-fade-up overflow-hidden flex flex-col max-h-[90vh]">
        ...
        {/* Invoice Body - Forced Black text on White background */}
        <div id="invoice-content" className="p-10 overflow-y-auto flex-1 bg-white text-black print:p-0 print:bg-white print:text-black shadow-inner">
          ...
          <table className="w-full text-right mb-10 text-black">
            <thead className="border-b-2 border-black">
              <tr className="text-xs uppercase font-black">
                <th className="py-2 px-1">المادة</th>
            ...
            <tbody className="divide-y divide-gray-200">
              {items.map((item, idx) => (
                <tr key={idx} className="font-bold">
                  <td className="py-4 px-1">{item.name}</td>
                  <td className="py-4 px-1 text-center tabular-nums">{item.quantity}</td>
                  <td className="py-4 px-1 tabular-nums">{item.price.toLocaleString()}</td>
                  <td className="py-4 px-1 font-black tabular-nums">{(item.price * item.quantity).toLocaleString()}</td>
                </tr>
              ))}
            </tbody>
          </table>
          ...
          <div className="border-t-2 border-black pt-6">
            <div className="flex justify-between items-center text-3xl font-black text-black">
        ...
        <div className="p-6 bg-gray-900/80 border-t border-gray-700 grid grid-cols-1 sm:grid-cols-3 gap-3 no-print">
            <button 
              onClick={handlePrint} 
              className="bg-[#FA8072] hover:bg-orange-500 text-white py-3.5 rounded-2xl font-black text-xs flex items-center justify-center gap-2 transition-all shadow-xl active:scale-95"
            >
                <Printer size={18} />
                طباعة PDF
            </button>
            <button 
              ...
              className="bg-green-600 hover:bg-green-500 text-white py-3.5 rounded-2xl font-black text-xs flex items-center justify-center gap-2 transition-all shadow-xl active:scale-95"
            >
            ...
            <button 
              onClick={handleCopy} 
              className={`py-3.5 rounded-2xl font-black text-xs flex items-center justify-center gap-2 transition-all border border-gray-700 ${copied ? 'bg-blue-600 text-white border-blue-500' : 'bg-gray-800 text-gray-400 hover:text-white'}`}
            >
                {copied ? <Check size={18} /> : <Copy size={18} />}
                {copied ? 'تم النسخ' : 'نسخ نصي'}
            </button>
        </div>
      </div>
    </div>
